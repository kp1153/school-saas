    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          ...
          <p className="text-2xl font-bold text-gray-900">₹{summary.total_pending || 0}</p>
        </div>
        <div className="bg-white rounded-lg p-6 border border-gray-200">
          <p className="text-sm text-gray-500">Total Collected</p>
          <p className="text-2xl font-bold text-green-600">₹{summary.total_collected || 0}</p>
        </div>
      </div>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {allFees.map((fee) => (
              <tr key={fee.id} className="hover:bg-gray-50">
                <td className="px-6 py-4 text-sm text-gray-900">{fee.student_name}</td>
