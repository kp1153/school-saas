...
export default async function DashboardPage() {
  ...
  const [pendingFees] = await db.select({ 
    ...
    count: sql`COUNT(*)`
  }).from(fees).where(sql`status = 'pending'`)
  const [paidFees] = await db.select({ 
    total: sql`SUM(amount)` 
  }).from(fees).where(sql`status = 'paid'`)
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <Link href="/students/add" className="bg-indigo-600 text-white px-5 py-2.5 rounded-lg hover:bg-indigo-700 transition text-sm font-medium shadow-sm">
          + Add New Student
        </Link>
      </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        <Link href="/students" className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:border-indigo-300 transition">
          <div className="text-2xl mb-3">🎓</div>
          <div className="font-semibold text-gray-900">Manage Students</div>
          <div className="text-sm text-gray-500 mt-1">View, add, edit students</div>
        </Link>
        <Link href="/teachers" className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:border-indigo-300 transition">
          <div className="text-2xl mb-3">👨‍🏫</div>
          <div className="font-semibold text-gray-900">Manage Teachers</div>
          <div className="text-sm text-gray-500 mt-1">View, add, edit teachers</div>
        </Link>
        <Link href="/fees" className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:border-indigo-300 transition">
          <div className="text-2xl mb-3">💰</div>
          <div className="font-semibold text-gray-900">Fee Management</div>
          <div className="text-sm text-gray-500 mt-1">Track and collect fees</div>
        </Link>
      </div>
    </div>
  )
